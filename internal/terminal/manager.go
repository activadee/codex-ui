...
func (m *Manager) CloseAll() {
	m.mu.Lock()
	list := make([]*session, 0, len(m.terms))
	for _, s := range m.terms {
		list = append(list, s)
	}
	m.terms = map[int64]*session{}
	m.mu.Unlock()
	for _, s := range list {
		if s == nil {
			continue
...
func (m *Manager) forward(s *session) {
	defer func() {
		...
		close(s.done)
		m.mu.Lock()
		if cur, ok := m.terms[s.threadID]; ok && cur == s {
			delete(m.terms, s.threadID)
		}
		m.mu.Unlock()
		status := "exited"
		if s.cmd.ProcessState != nil && !s.cmd.ProcessState.Success() {
			status = fmt.Sprintf("exit:%d", s.cmd.ProcessState.ExitCode())
		}
		m.emitExit(s.threadID, status)
	}()
	buf := make([]byte, 4096)
	for {
