...
func (s *Service) Ensure(threadID int64, worktree string) {
    ...
    if worktree == "" { return }
    s.mu.Lock()
    if w, ok := s.watchers[threadID]; ok { s.mu.Unlock(); _ = w.Add(worktree); return }
    watcher, err := fsnotify.NewWatcher()
    if err != nil { s.mu.Unlock(); fmt.Printf("watcher create %d: %v\n", threadID, err); return }
    s.watchers[threadID] = watcher
    s.mu.Unlock()
    if err := addRecursive(watcher, worktree); err != nil { fmt.Printf("watcher setup %d: %v\n", threadID, err) }
    go s.observe(threadID, watcher)
}

func (s *Service) Remove(threadID int64) {
    s.mu.Lock()
    if t, ok := s.notifyTimers[threadID]; ok { t.Stop(); delete(s.notifyTimers, threadID) }
    w, ok := s.watchers[threadID]
    if ok { delete(s.watchers, threadID) }
    s.mu.Unlock()
    if ok { _ = w.Close() }
}
...
func (s *Service) Stop() {
    ...
    for _, t := range s.notifyTimers { timers = append(timers, t) }
    ws := make([]*fsnotify.Watcher, 0, len(s.watchers))
    for _, w := range s.watchers { ws = append(ws, w) }
    s.notifyTimers = map[int64]*time.Timer{}
    s.watchers = map[int64]*fsnotify.Watcher{}
    s.mu.Unlock()
    for _, t := range timers { if t!=nil { t.Stop() } }
    for _, w := range ws { if w!=nil { _ = w.Close() } }
}
...
func addRecursive(w *fsnotify.Watcher, root string) error {
    return filepath.WalkDir(root, func(path string, d os.DirEntry, err error) error {
        ...
        if err := w.Add(path); err != nil { fmt.Printf("watcher add %s: %v\n", path, err) }
        return nil
    })
}
...
func (s *Service) observe(threadID int64, w *fsnotify.Watcher) {
    for {
        ...
        case ev, ok := <-w.Events:
            if !ok { return }
            if isIgnored(ev.Name) { continue }
            if ev.Op & fsnotify.Create != 0 { if info, err := os.Stat(ev.Name); err==nil && info.IsDir() { _ = addRecursive(w, ev.Name) } }
            s.schedule(threadID)
...
func isIgnored(path string) bool {
    ...
    if strings.Contains(path, string(filepath.Separator)+".git"+string(filepath.Separator)) { return true }
    return strings.HasSuffix(path, string(filepath.Separator)+".git")
}

func (s *Service) schedule(threadID int64) {
    s.mu.Lock()
    if t, ok := s.notifyTimers[threadID]; ok { t.Stop() }
    var t *time.Timer
    t = time.AfterFunc(200*time.Millisecond, func(){
        if s.onDiff != nil { s.onDiff(threadID) }
        s.mu.Lock()
        if cur, ok := s.notifyTimers[threadID]; ok && cur == t { delete(s.notifyTimers, threadID) }
        s.mu.Unlock()
    })
