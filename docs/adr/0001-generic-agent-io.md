...
- **Adapter registry** – `internal/agents/connector` defines interfaces, DTOs, and helper factories. Each adapter registers itself with metadata (id, default capabilities, launch hints) sourced from `agents.yaml` and loaded through `internal/config/agents.go`, which also expands `${VAR}` tokens within `env` entries before hydrating the registry.
